...
logger = logging.getLogger('GAN.VAE_GAN')
...
class Discriminator(Module):
    ...
    def __init__(self, in_channels, hidden_dim, num_layers, module_name, dropout, device):
        super(Discriminator, self).__init__()
        if module_name == 'gru':
            self.rnn = nn.GRU(in_channels, hidden_dim, num_layers, batch_first=True, dropout=dropout)
        else:
...
def timegan(train_data, ori_data, params, restore_iteration=-2):
    """TimeGAN function.
    ...
    train_data, _, _ = MinMaxScaler(train_data, min_val, max_val)
    ...
    dataset = utils.TS_sampler(train_data)
    ...
    for itt in tqdm(range(start_iteration, iterations)):
        ...
        Y_logits_fake, Y_fake = D(vae.decode(vae.reparameterize(mu, log_var).detach()))
        gan_loss = G_loss_U(input=Y_logits_fake, target=torch.ones_like(Y_fake))
        (loss['loss'] + 0.0001 * gan_loss).backward()
        vae_optim.step()
        ...
        if itt % 10 == 0:
            ...
            try:  # https://stackoverflow.com/a/58876890/8365622
                ...
                it = iter(dataloader)
                x_mb = next(it).to(device)
            Y_logits_fake, Y_fake = D(x_mb.detach())
            Y_logits_real, Y_real = D(recons.detach())

            D_optim.zero_grad()
            step_d_loss_real = D_loss_real(input=Y_logits_real, target=torch.ones_like(Y_real))
            step_d_loss_fake = D_loss_fake(input=Y_logits_fake, target=torch.zeros_like(Y_fake))
            step_d_loss = step_d_loss_real + step_d_loss_fake

            if step_d_loss > 0.15:
                step_d_loss.backward()
                D_optim.step()
        else:
            step_d_loss = torch.zeros(5, device=device)[1]
        ...
        if itt % 100 == 0:
            logger.info(f'step: {itt}/{iterations}, loss: {loss["loss"].item():.4f},'
                        ...
                        f'Generator GAN loss: {0.0001 * gan_loss.item():.4f}, Dis GAN loss: {step_d_loss.item():.4f}')
