"""
Modified version of TimeGAN in order to support conditional inputs. This is cTimeGAN in our paper.
"""
...
logger = logging.getLogger('GAN.cTSGAN')
...
class Timegan:
    def __init__(self, train_data, ori_data, params):
        ...
        self.noise_std = params.model_param.noise_std
        self.params = params
        ...
        for itt in tqdm(range(start_iteration, self.iterations)):
            ...
            for kk in range(2):
                ...
                step_g_loss = step_g_loss_u + self.gamma * step_g_loss_u_e + \
                              100 * torch.sqrt(step_g_loss_s) + 100 * step_g_loss_v
                ...
                loss_cache[itt_cache, 5] = step_e_loss0.item()
                loss_cache[itt_cache, 6] = 0.1 * step_g_loss_s.item()
                step_e_loss.backward()
                self.E_optim.step()
                self.R_optim.step()
            ...
            if step_d_loss > 0.15:
                loss_cache[itt_cache, 7] = step_d_loss_real.item()
                loss_cache[itt_cache, 8] = step_d_loss_fake.item()
                loss_cache[itt_cache, 9] = step_d_loss_fake_e.item()
                step_d_loss.backward()
                self.D_optim.step()
            else:
                loss_cache[itt_cache, 7:] = 0
...
def plot_all_loss(loss_summary, plot_num, params, location='./figures/'):
    ...
    loss_list = ['step_g_loss_u', 'step_g_loss_u_e', 'step_g_loss_s', 'step_g_loss_v1', 'step_g_loss_v2']
